...
def draw_boxes(frame: np.ndarray, detections: List[dict]) -> np.ndarray:
    """Draw bright neon bounding boxes with dark label backgrounds."""
    out = frame.copy()
    ...
    for det in detections:
        ...
        base_color = get_color_for_class(cls)
        # Boost brightness for a "neon" effect
        neon_color = tuple(min(255, int(c * 1.4)) for c in base_color)
        
        # Bounding box
        cv2.rectangle(out, (x1, y1), (x2, y2), neon_color, 2)

        # Label background (Dark gray/blue solid color for readability)
        label = f"{cls} {conf:.0%}"
        (tw, th), bl = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
        cv2.rectangle(out, (x1, y1 - th - bl - 6), (x1 + tw + 6, y1), (15, 15, 20), -1)
        
        # Neon text
        cv2.putText(
            out, label, (x1 + 3, y1 - bl - 2),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, neon_color, 1, cv2.LINE_AA,
        )
    return out
